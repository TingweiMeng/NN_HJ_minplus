...
t0 = args.t0
Ndata1d = 100
batchsize = 25   # set it to be small if Lx is larger
...
def ode_sourceterm(P, q, r, dt, t):
    ...
    multi = -tf.multiply(dt, expt)/8
    dr = tf.multiply(multi, dr)
    # dq = dt (q/4 - e^t P'q/4)
    dq = tf.matmul(tf.transpose(P,perm=[0,1,3,2]), q)
    dq = tf.subtract(q/4, tf.multiply(expt, dq) /4)
    dq = tf.multiply(dt, dq)
    # dP = dt(-e^tP'P/4 + P/2 +e^{-t}I/4)
    dP = tf.matmul(tf.transpose(P,perm=[0,1,3,2]), P)
    dP = tf.subtract(P/2, tf.multiply(dP, expt)/4)
    dP = tf.add(dP, tf.multiply(expmt/4, tf.eye(n, dtype=tf.dtypes.float64)))
...
def dtAxpBu(x_in, dt, P_in, q_in, t):
    ...
    dx = tf.subtract(x_in/4, tf.multiply(dx, expt)/4)
...
if ploterrflag:
    ...
    pde_err = -dSkdt + tf.reduce_sum(dSkdx*dSkdx * np.exp(t0)/8 - x_ph * x_ph *np.exp(-t0) /8 - dSkdx*x_ph/4, axis=-2, keepdims=True)
...
if plotSflag:
    ...
    print('printS ends {}'.format(filenameS))
